...
class PDFGenerator:
    ...
    def _build_header(self, content: Dict[str, Any]) -> List:
        ...
        elements.append(HRFlowable(
            width='100%', thickness=1, color=self.COLOR_BORDER,
    ...
    def _build_patient_info(self, patient: Dict[str, Any]) -> List:
        ...
        # Create info table
        data = [
            ['Name:', patient.get('name', 'N/A'), 'Gender:', patient.get('gender', 'N/A')],
            ['Age:', str(patient.get('age', 'N/A')), 'Scheduled Visit:', patient.get('scheduled_visit', 'N/A')],
        ]
    ...
    def _build_checkin_trend(self, trend: Dict[str, Any]) -> List:
        ...
        elements.append(Paragraph(f'5. Recent Status Trend (Past {days} Days)', self.styles['SectionHeading']))
        ...
        if checkin_count == 0:
            elements.append(Paragraph('No check-in data available for this period.', self.styles['MutedText']))
            return elements
