...
from app.utils.deps import get_current_active_user, get_current_doctor, get_current_patient
...
async def register(request: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    if request.user_type == UserType.PATIENT:
        patient = Patient(user_id=user.id, first_name=request.first_name, last_name=request.last_name)
        db.add(patient)
    elif request.user_type == UserType.DOCTOR:
        doctor = Doctor(user_id=user.id, first_name=request.first_name, last_name=request.last_name)
        db.add(doctor)
    ...
    # Generate token
    token = create_access_token({"sub": str(user.id), "type": user.user_type.value})

    return TokenResponse(access_token=token, user_type=user.user_type, user_id=user.id)
...
async def get_current_patient_profile(patient: Patient = Depends(get_current_patient)):
...
async def get_current_doctor_profile(doctor: Doctor = Depends(get_current_doctor)):
...
async def reset_password(request_body: PasswordResetConfirm, db: AsyncSession = Depends(get_db)):
    """
