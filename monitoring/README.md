...
| 服务         | URL                   | 默认凭据                 |
| ------------ | --------------------- | ------------------------ |
| Grafana      | http://localhost:3001 | admin / heartguardian123 |
...
│   └── dashboards/
│       └── heartguardian-overview.json  # 主 Dashboard
│
├── alertmanager/
...
| 指标名                                        | 类型      | 描述               |
| --------------------------------------------- | --------- | ------------------ |
| `heartguardian_http_requests_total`           | Counter   | HTTP 请求总数      |
| `heartguardian_http_request_duration_seconds` | Histogram | 请求延迟分布       |
| `heartguardian_http_requests_in_progress`     | Gauge     | 当前进行中的请求数 |

### AI 引擎指标

| 指标名                                      | 类型      | 描述            |
| ------------------------------------------- | --------- | --------------- |
| `heartguardian_ai_requests_total`           | Counter   | AI API 请求总数 |
| `heartguardian_ai_request_duration_seconds` | Histogram | AI 请求延迟     |
| `heartguardian_ai_tokens_total`             | Counter   | AI token 使用量 |

### 风险检测指标

| 指标名                                 | 类型    | 描述               |
| -------------------------------------- | ------- | ------------------ |
| `heartguardian_risk_events_total`      | Counter | 检测到的风险事件   |
| `heartguardian_risk_events_unreviewed` | Gauge   | 未审查的风险事件数 |

### 业务指标

| 指标名                                | 类型    | 描述             |
| ------------------------------------- | ------- | ---------------- |
| `heartguardian_checkins_total`        | Counter | 每日检查提交数   |
| `heartguardian_assessments_total`     | Counter | 评估完成数       |
| `heartguardian_average_mood_score`    | Gauge   | 平均情绪分数     |
| `heartguardian_websocket_connections` | Gauge   | WebSocket 连接数 |

### 数据库指标

| 指标名                                    | 类型      | 描述         |
| ----------------------------------------- | --------- | ------------ |
| `heartguardian_db_pool_size`              | Gauge     | 连接池大小   |
| `heartguardian_db_pool_checked_out`       | Gauge     | 已使用连接数 |
| `heartguardian_db_query_duration_seconds` | Histogram | 查询延迟     |
