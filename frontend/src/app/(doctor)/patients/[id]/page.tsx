  ...
  return (
    <div className="space-y-6">
      ...
      <Breadcrumb className="mb-2">
        <BreadcrumbItem>
          <BreadcrumbLink href="/patients">{t('title')}</BreadcrumbLink>
        </BreadcrumbItem>
        <BreadcrumbSeparator />
        <BreadcrumbItem>
          <BreadcrumbPage>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-success text-success-foreground rounded-lg hover:bg-success/90 transition-colors font-medium text-sm"
          >
              ...
              <div className="flex items-center gap-4 text-primary-foreground/80 text-sm mt-1">
                {profile.date_of_birth && (
                  <span className="flex items-center gap-1">
                    ...
                    {t('yearsOld', { defaultValue: 'years old' })}
                  </span>
                )}
      ...
      {/* Medical Information */}
      {profile &&
        (profile.current_medications || profile.medical_conditions || profile.allergies) && (
          <Disclosure defaultOpen={true}>
            ...
            <DisclosurePanel className="space-y-4 border-t border-border">
              {profile.current_medications && (
                <div>
                  <h4 className="text-sm font-medium text-foreground mb-1">
                    {t('medications', { defaultValue: 'Current Medications' })}
                  </h4>
              ...
              {profile.medical_conditions && (
                <div>
                  <h4 className="text-sm font-medium text-foreground mb-1">
                    {t('conditions', { defaultValue: 'Medical Conditions' })}
                  </h4>
                  <p className="text-muted-foreground bg-muted p-3 rounded-lg whitespace-pre-wrap">
                    {profile.medical_conditions}
                  </p>
              ...
              {profile.allergies && (
                <div>
                  ...
                  <p className="text-muted-foreground bg-destructive/10 p-3 rounded-lg whitespace-pre-wrap">
                    {profile.allergies}
                  </p>
                </div>
              )}
          ...
          <Disclosure defaultOpen={true}>
            <DisclosureButton>
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-foreground">
                  {t('mentalHealthContext', { defaultValue: 'Mental Health Context' })}
                </span>
              </div>
            </DisclosureButton>
            ...
            <DisclosurePanel className="space-y-4 border-t border-border">
              ...
              {profile.mental_health_goals && (
                <div>
                  ...
                  <p className="text-muted-foreground bg-blue-500/10 p-3 rounded-lg whitespace-pre-wrap">
                    {profile.mental_health_goals}
                  </p>
                </div>
              )}
              {profile.support_system && (
                <div>
                  ...
                  <p className="text-muted-foreground bg-green-500/10 p-3 rounded-lg whitespace-pre-wrap">
                    {profile.support_system}
                  </p>
                </div>
              )}
              {profile.triggers_notes && (
                <div>
                  ...
                  <p className="text-muted-foreground bg-orange-500/10 p-3 rounded-lg whitespace-pre-wrap">
                    {profile.triggers_notes}
                  </p>
                </div>
              )}
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground mt-1">
                        ...
                        <span>
                          {t('sleep')}: {c.sleep_hours}h
                        </span>
                        <span>
                          {t('quality')}: {getSleepQualityLabel(c.sleep_quality)}
                        </span>
                        <span>
                          {t('medication')}:{' '}
                          {c.medication_taken ? (
