    ...
    <div className="h-full overflow-y-auto p-4 space-y-4 pb-24">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t('subtitle', { defaultValue: 'Track your well-being' })}</p>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{t('mood', { defaultValue: 'Mood' })}</p>
                </div>
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{t('sleep', { defaultValue: 'Sleep' })}</p>
                </div>
                <div className="text-center">
                  <div className={`flex items-center gap-1 ${todayCheckin.medication_taken ? 'text-green-600' : 'text-red-500'}`}>
                    <Pill className="w-4 h-4" />
                    <span className="font-medium">{todayCheckin.medication_taken ? '✓' : '✗'}</span>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">{t('meds', { defaultValue: 'Meds' })}</p>
                </div>
              </div>
              <div className="flex items-center gap-1 text-green-600 dark:text-green-400">
                <CheckCircle2 className="w-4 h-4" />
                <span className="text-xs font-medium">{t('checkedIn', { defaultValue: 'Checked in' })}</span>
              </div>
            </div>
          ) : (
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-sm">{t('notCheckedIn', { defaultValue: "You haven't checked in today" })}</span>
              </div>
              <Link
                href="/checkin"
        ...
        <Link href="/checkin">
          <Card className="h-full border-border/50 shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
            <CardContent className="p-4 flex flex-col items-center text-center">
              ...
              <h3 className="font-semibold text-foreground">{t('dailyCheckin', { defaultValue: 'Daily Check-in' })}</h3>
              <p className="text-xs text-muted-foreground mt-1">{t('dailyCheckinDesc', { defaultValue: 'Log mood, sleep & meds' })}</p>
            </CardContent>
          </Card>
        </Link>
        ...
        <Link href="/assessment">
          <Card className="h-full border-border/50 shadow-sm hover:shadow-md transition-shadow cursor-pointer group">
            <CardContent className="p-4 flex flex-col items-center text-center">
              ...
              <h3 className="font-semibold text-foreground">{t('assessment', { defaultValue: 'Assessment' })}</h3>
              <p className="text-xs text-muted-foreground mt-1">{t('assessmentDesc', { defaultValue: 'PHQ-9, GAD-7, PCL-5' })}</p>
            </CardContent>
          </Card>
        </Link>
      </div>
      ...
      {recentCheckins.length > 0 && (
        <Card className="border-border/50 shadow-sm">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base font-medium">{t('recentCheckins', { defaultValue: 'Recent Check-ins' })}</CardTitle>
              <Link href="/checkin" className="text-xs text-primary hover:underline flex items-center gap-1">
            ...
            <div className="divide-y divide-border">
              {recentCheckins.slice(0, 5).map((checkin) => (
                <div key={checkin.id} className="py-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-foreground">{formatDate(checkin.checkin_date)}</p>
                    <p className="text-xs text-muted-foreground">
                      {checkin.sleep_hours}h {t('sleep', { defaultValue: 'sleep' })} · {sleepLabels[checkin.sleep_quality - 1]}
                    </p>
                  </div>
      ...
      {recentAssessments.length > 0 && (
        <Card className="border-border/50 shadow-sm">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base font-medium">{t('recentAssessments', { defaultValue: 'Recent Assessments' })}</CardTitle>
              <Link href="/assessment" className="text-xs text-primary hover:underline flex items-center gap-1">
            ...
            <div className="divide-y divide-border">
              {recentAssessments.map((assessment) => (
                <div key={assessment.id} className="py-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-foreground">{assessment.assessment_type}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${getSeverityColor(assessment.severity)}`}>
      ...
      {recentCheckins.length === 0 && recentAssessments.length === 0 && (
        <Card className="border-border/50 shadow-sm">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="font-semibold text-foreground mb-2">{t('getStarted', { defaultValue: 'Get Started' })}</h3>
            <p className="text-sm text-muted-foreground mb-4">
              {t('getStartedDesc', { defaultValue: 'Start tracking your mental health by completing a daily check-in or taking an assessment.' })}
            </p>
