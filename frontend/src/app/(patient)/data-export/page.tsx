...
const statusConfig: Record<ExportStatus, { icon: typeof Clock; color: string; bgColor: string }> = {
  ...
  PROCESSING: { icon: Loader2, color: 'text-info', bgColor: 'bg-info/10' },
  ...
  DOWNLOADED: { icon: CheckCircle2, color: 'text-success', bgColor: 'bg-success/10' },
}
...
export default function DataExportPage() {
  ...
  const fetchExportRequests = useCallback(async () => {
    try {
      ...
      const processingExport = data.find((e) => e.status === 'PENDING' || e.status === 'PROCESSING')
      if (processingExport) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('429') || errorMessage.includes('rate')) {
        setError(
          t('rateLimitError', {
            defaultValue: 'You can only request one export per 24 hours. Please try again later.',
          })
        )
      } else {
          ...
          <div className="flex gap-3">
            ...
            <div className="text-sm text-blue-800 dark:text-blue-300">
              <p className="font-medium mb-1">
                {t('infoTitle', { defaultValue: 'Your Data Rights' })}
              </p>
              <p>
                {t('infoDesc', {
                  defaultValue:
                    'You have the right to receive a copy of your personal data. Exports are available once every 24 hours and download links expire after 7 days.',
                })}
              </p>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-4 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {formState.export_format === 'JSON' &&
                  t('formatJsonDesc', { defaultValue: 'Complete structured data in JSON format' })}
                {formState.export_format === 'CSV' &&
                  t('formatCsvDesc', {
                    defaultValue: 'Spreadsheet-compatible CSV files in ZIP archive',
                  })}
                {formState.export_format === 'PDF_SUMMARY' &&
                  t('formatPdfDesc', { defaultValue: 'Human-readable summary report' })}
              </p>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormState((prev) => ({ ...prev, date_from: e.target.value || undefined }))
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormState((prev) => ({ ...prev, date_to: e.target.value || undefined }))
                    }
                    className="w-full border border-input bg-background text-foreground rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                  />
                </div>
            ...
            <div className="flex items-start gap-2 text-xs text-muted-foreground bg-muted/50 p-3 rounded-lg">
              ...
              <p>
                {t('securityNotice', {
                  defaultValue:
                    'Your export will be encrypted and available for 7 days. Download links can only be used 3 times.',
                })}
              </p>
            </div>
            ...
            <Button
              ...
              disabled={
                submitting ||
                !Object.values(formState).some((v) => v === true && typeof v === 'boolean')
              }
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-foreground">
                              {request.export_format === 'PDF_SUMMARY'
                                ? 'PDF Summary'
