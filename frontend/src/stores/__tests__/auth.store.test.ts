...
import { useAuth, getRedirectPath, isRouteAllowed } from '../auth.store';
...
describe('Auth Store', () => {
  ...
  describe('clearPasswordMustChange', () => {
    it('clears the passwordMustChange flag', () => {
      useAuth.setState({ passwordMustChange: true });

      const { clearPasswordMustChange } = useAuth.getState();
      act(() => {
        clearPasswordMustChange();
      });

      expect(useAuth.getState().passwordMustChange).toBe(false);
    });
  });
});
